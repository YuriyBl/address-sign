	...
	static fromMesh = function (mesh: THREE.Mesh, objectIndex?: object) {
		...
		for (let i = 0; i < csg.polygons.length; i++) {
			...
			for (let j = 0; j < p.vertices.length; j++) {
				let v = p.vertices[j];
				v.pos.copy(CSG.ttvv0.copy(new THREE.Vector3(v.pos.x, v.pos.y, v.pos.z)).applyMatrix4(mesh.matrix));
				v.normal.copy(CSG.ttvv0.copy(new THREE.Vector3(v.normal.x, v.normal.y, v.normal.z)).applyMatrix3(CSG.tmpm3));
			}
		}
		return csg;
	};

	static fromGroup = function (group: THREE.Group) {
		let csg = new CSG();
		group.traverse((object) => {
			if (object instanceof THREE.Mesh) {
				object.updateMatrix();
				csg = csg.union(CSG.fromMesh(object));
			}
		});
		return csg;
	};
