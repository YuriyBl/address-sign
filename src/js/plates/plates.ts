...
export class Color {
	public static list: Color[] = [new Color('White', 0xffffff, 0xdddddd), new Color('Yellow', 0xffe659, 0x826e00)];

	constructor(public name: string, public value: number, public lightValue: number) {}
}

export class Size {
	public static list: Size[] = [new Size('S', 0.7), new Size('M', 1), new Size('L', 1.5)];

	constructor(public name: string, public mult: number) {}
...
class Plate {
	...
	static genLine(params: IPlateParams): THREE.Mesh {
		const lineMesh = new THREE.Mesh(new THREE.BoxBufferGeometry(params.lineThikness, params.numSize, params.depth));
	...
	static genPlates(
		...
		const backPlateMesh = new THREE.Mesh(roundedRectGeometry(plateWidth, plateHeight, params.depth!, params.radius!));
	...
	new Plate(
		...
		(params) =>
			async (input: IAppInput): Promise<[THREE.Mesh, THREE.Mesh, THREE.Mesh, THREE.Group]> => {
				...
				engraveGroup.traverse((o) => {
					o.translateX(-plateWidth / 2);
